...
REST や RPC のような仕組みと低水準レイヤーを組み合わせればシステムプログラミング言語がなくても可能だと思う。欠点はオーバーヘッドなのは仕方ない (低遅延プロトコルの開発が必要)。 VAX/VMS はシステムコールまわりが統一されていた話 (Common Language Environment) も云々ある。この方式ならば開発言語を問わないのかもしれない。
