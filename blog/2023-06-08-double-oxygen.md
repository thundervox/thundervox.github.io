...
REST や RPC のような仕組みと低水準レイヤーを組み合わせればシステムプログラミング言語がなくても可能だと思う。互換性についても既存ネイティブアプリケーションのクラウドサービス化など進んでいくだろうから重視しない。どの観点から利点を説明したらいいかは低水準処理の知識を用いなくても手軽に扱えることだろうね(軽トラみたいなものかな)。欠点はオーバーヘッドなのは仕方ない (超低遅延プロトコルやネットワーク機器の開発が必要)。VAX/VMS はシステムコールまわりが統一されていた話 (Common Language Environment) も云々ある。この方式ならば開発言語を問わないのかもしれない。
